...
public class StudentInfoUtil {
    ...
    public static String[] getStudentIntention(String[] intentionCity, String[] intentionIndustry, String[] intentionFunction){
        String[] intention = new String[cities.length];
        for(int i = 0;i < intention.length;i++){
            intention[i] = "0";
        }
        for(int i = 0;i < intentionCity.length;i++){
            for(int j = 0;j < cities.length;j++){
                if(intentionCity[i].equals(cities[j])){
                    intention[j] = "1";
                }
            }
        }
